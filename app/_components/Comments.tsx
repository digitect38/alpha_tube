...
export function Comments({ videoId }: { videoId: string }) {
  ...
  const load = () => {
    fetch(`/apps/video/api/videos/${videoId}/comments`)
      .then(r => r.json())
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!body.trim()) return;
    setBusy(true);
    const r = await fetch(`/apps/video/api/videos/${videoId}/comments`, {
      method: 'POST',
        ...
        {items.map(c => (
          <li key={c.id}>
            <div className="text-sm">
              <a href={`/apps/video/channel/${c.author.handle}`} className="font-medium hover:underline">
