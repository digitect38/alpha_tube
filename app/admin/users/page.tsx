...
export default function AdminUsers() {
  ...
  useEffect(() => {
    fetch('/apps/video/api/auth/me').then(r => r.json()).then(d => setMeId(d.user?.id ?? null));
    fetch('/apps/video/api/admin/users').then(r => r.json()).then(d => setRows(d.users ?? []));
  }, []);

  const toggle = async (u: Row) => {
    setBusy(u.id); setErr(null);
    const r = await fetch('/apps/video/api/admin/users', {
      method: 'PATCH',
